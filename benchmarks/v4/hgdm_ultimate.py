...
# V6: High-Performance Kernels
from kernel_scan import chunkwise_hgdm_forward
...
class MultiHeadGatedDelta(nn.Module):
    ...
    def forward(self, x, state: Optional[torch.Tensor] = None):
        ...
        if T > 1 and state is None:
            # V6: Chunkwise Parallelism with Gradient Checkpointing
            out, last_state = chunkwise_hgdm_forward(q, k, v, alpha, beta, chunk_size=256)
            out = out * out_gate
