...
fixed-size state forward. On Enwik8, a 122\,M HGDM with Cross-Layer State Fusion
...
exhibiting linear training scaling and constant inference-time memory. After curriculum fine-tuning, HGDM achieves 100\% passkey retrieval accuracy at 32{,}768 tokens.
\end{abstract}
...
\begin{itemize}
  ...
  \item achieves 100\% passkey retrieval accuracy up to 32{,}768 context length after curriculum fine-tuning,
...
The 122\,M baseline uses: $d = 768$, $L = 12$ layers, $H = 12$ heads, $d_k = d_v = 64$, $d_{\text{ff}} = 3072$, and $L_{\max} = 2048$. Total parameters $\approx 122$\,M.
...
\end{equation}
Here, the softplus bias value $b_\Delta$ is initialized to $0.5413$ so that $\mathrm{softplus}(0.5413) + \eps_0 \approx 1.001$ at step 0. This ensures that the continuous timescale step size $\Delta t$ starts near $1.0$ at the onset of training, preserving the cortical timescale prior at initialization.
...
An HGDM layer processes length $T$ sequences in $O(T d_k d_v)$ total sequential operations (or $O((T/C) \log C)$ total depth with $O(T)$ work under parallel scan with chunk size $C$) with $O(d_k d_v)$ persistent memory per head, ensuring constant memory footprint during autoregressive generation.
\end{proposition}

\begin{proposition}[Bounded State Norms]
Let the state sequence update according to Equation~\eqref{eq:state_update}. Assume $\alpha_t \in (0, \alpha_{\max}]$ with $\alpha_{\max} < 1$, $\|\beta_t\| \leq \beta_{\max}$, $\|k_t\|_2 \leq \|k\|_2$, and $\|v_t\|_2 \leq \|v\|_2$. Under these assumptions, the Frobenius norm of the recurrent state is bounded for all $t$ via the geometric series:
\begin{equation}
  \norm{S_t}_F \leq \frac{\beta_{\max}\,\norm{k}_2\,\norm{v}_2}{1 - \alpha_{\max}}.
\end{equation}
When evaluating this bound using the empirically observed post-training maximum decay parameter $\alpha_{\max} = 0.999875$ (corresponding to the $\tau=8{,}000$ head), $\beta_{\max} = 1.0$, and RMSNorm-bounded feature magnitudes $\norm{k}_2 \approx \sqrt{d_k} = 8$, $\norm{v}_2 \approx \sqrt{d_v} = 8$, we obtain a finite mathematical upper bound of $\norm{S_t}_F \leq 64 / 0.000125 = 512{,}000 < \infty$. We emphasize that this bound holds post-training based on the learned decay parameters and does not mathematically constrain $\alpha_t$ during the optimization process itself.
\end{proposition}
...
We apply a weight decay coefficient of $0.01$ across all non-bias parameters. Pre-activation RMSNorm layers with an added epsilon of $10^{-8}$ stabilize the optimization trajectory, and a dropout rate of $0.1$ is applied to the feed-forward outputs.
...
\begin{table}[!ht]
\centering
\caption{Enwik8 snapshot comparison. All HGDM and Transformer BPB values are on a held-out validation set via \texttt{evaluate\_model}, 50 random batches of batch size 4 ($\text{SE} \approx 0.012$). The \emph{primary} Transformer comparison is Transformer\textsuperscript{\textdagger} (weight tying, SwiGLU, RMSNorm), which matches HGDM's component choices. The vanilla Transformer (no tying, SiLU, LayerNorm) is included as a secondary reference. \textbf{Note: literature baselines are near-convergence results trained for orders of magnitude more tokens; they are listed as long-run targets only and are not directly comparable to the 1{,}000-step snapshots above the divider.}}
\label{tab:enwik8}
\begin{tabular}{lrrrr}
\toprule
Model & BPB $\downarrow$ & Peak memory (GB) & Steps/sec & Tokens Trained \\
\midrule
\multicolumn{5}{l}{\textit{1{,}000-step snapshots (this work)}} \\
Transformer\textsuperscript{\textdagger} (122M, Tied+SwiGLU) \textbf{[primary]} & 2.45 & 3.42 & 1.58 & 24.6\,M \\
Transformer (122M, vanilla)                            & 3.68 & 3.65 & 1.69 & 24.6\,M \\
HGDM (122M)                                           & 1.85 & 4.44 & 1.12 & 24.6\,M \\
HGDM + Fusion                                         & \textbf{1.72} & 4.45 & 1.11 & 24.6\,M \\
\midrule
\multicolumn{5}{l}{\textit{Literature baselines (near convergence --- not directly comparable)}} \\
MambaByte (120M) \cite{wang2024mambabyte}  & 1.34 & --- & --- & $\approx$250\,B \\
MEGABYTE (120M) \cite{yu2023megabyte}      & 1.48 & --- & --- & $\approx$40\,B \\
RWKV-4 (120M) \cite{peng2023rwkv}          & 1.42 & --- & --- & $\approx$330\,B \\
\bottomrule
\end{tabular}
\end{table}

We report the training snapshot after 1{,}000 update steps in Table~\ref{tab:enwik8}. HGDM + Fusion achieves 1.72\,BPB, outperforming the Tied+SwiGLU Transformer (2.45\,BPB) by 0.73\,BPB under equal compute. While HGDM's absolute training memory is moderately higher at $T=2{,}048$ ($4.44$\,GB vs.\ $3.42$\,GB) due to persistent recurrent state storage, its memory footprint remains strictly bounded and constant ($O(1)$) as sequence length scales (E2), whereas the Transformer baseline quickly encounters quadratic VRAM explosion and OOM errors at scale.
...
findings. The central thesis is that an architecture whose activity scales with input informativeness, not sequence
length, is both more faithful to biological computation and more efficient by
construction. HGDM represents a first step toward that architecture.
