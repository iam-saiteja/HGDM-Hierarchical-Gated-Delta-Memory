...
% \usepackage[preprint]{neurips_2024}   % Optional NeurIPS style: upload neurips_2024.sty to Overleaf or use the official template. Commented out to avoid Overleaf errors.
\usepackage{amsfonts}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{amsthm}
...
and $O(T/C)$ sequential depth with $O(C)$ parallel width per chunk, where $C$ is the chunk size. Importantly, inference runs with constant ($O(1)$) 
memory overhead by carrying only the fixed-size state forward; during training the 
fused kernel stores per-chunk intermediate states, yielding $O(T)$ memory. On Enwik8, a 120M HGDM reaches 1.72\,BPB after 
1{,}000 steps, outperforming a matched Transformer baseline (3.68\,BPB) under equal 
compute, while exhibiting linear training scaling and constant inference-time memory.
\end{abstract}
...
\begin{quote}
\textit{``The dominant sequence transduction models are based on complex
recurrent or convolutional neural networks\ldots We propose a new simple
network architecture, the Transformer, based solely on attention
mechanisms, dispensing with recurrence and convolutions entirely.''}
\hfill (Vaswani et al., 2017 \cite{vaswani2017attention})
\end{quote}

\noindent
Vaswani et al.\ dispensed with recurrence. In this work, we explore a recurrent
alternative that dispenses with self-attention.
...
\begin{enumerate}
  ...
  \item \textbf{Predictive error propagation}: only prediction errors
  drive updates; accurate predictions cost nothing.
  \item \textbf{Sparse activations}: most units are silent at any moment.
\end{enumerate}

We instantiate all three in a single trainable architecture, \textbf{HGDM},
...
\begin{table}[!ht]
\centering
\caption{Heuristic mapping inspired by Hasson et al.\ \cite{hasson2008hierarchy}, not directly measured values.}
\label{tab:trw}
\begin{tabular}{llll}
\toprule
...
RWKV \cite{peng2023rwkv}, and Gated Linear Attention (GLA) \cite{yang2023gla}, has revived interest in linear recurrences. GLA is closely related, leveraging scalar-gated outer-product state updates. Furthermore, 
...
The 120\,M baseline uses: $d = 768$, $L = 12$ layers, $H = 12$ heads, $d_k = d_v = 64$, $d_{\text{ff}} = 3072$, and $L_{\max} = 2048$. Total parameters $\approx 120$\,M.
...
\begin{tikzpicture}[
  ...
  operation/.style={draw, circle, fill=gray!10, inner sep=2pt, scale=0.85},
...
\end{equation}
Here, the softplus bias value $b_\Delta \approx 0.54132485$ is computed as the exact mathematical inverse of the softplus function at $1.0$ under offset $\eps_0 = 10^{-3}$, i.e., solving $\log(e^{1 - 10^{-3}} - 1) \approx 0.5413$. This exact initialization guarantees that the continuous timescale step size $\Delta t$ starts precisely at $1.0$ at step 0, preserving the cortical timescale prior at the onset of training.
...
An HGDM layer processes length $T$ sequences in $O(T d_k d_v)$ operations with $O(d_k d_v)$ persistent memory per head, ensuring constant memory footprint during autoregressive generation.
\end{theorem}
...
We compare three variants: (a) Full HGDM with multi-timescale $\tau \in \{4, 30, 200, 1200, 8000\}$; (b) Flat timescale $\tau=200$ for all heads; (c) Learned-only timescale initialized randomly. This experiment demonstrates that a neuroscience-inspired, multi-timescale initialization yields faster convergence and lower validation loss than flat or random timescale baselines, stabilizing optimization.
...
We train a single HGDM model from scratch on raw byte sequences representing three distinct modalities: PCM audio (16-bit), raw RGB pixels, and raw video frames. This experiment demonstrates that a single HGDM architecture can learn coherent representations directly across heterogeneous raw-byte modalities without any modality-specific tokenizers or custom embedding structures.
...
We examine the eigenvalues of the state decay matrices across all layers. This experiment verifies that all eigenvalues reside strictly within the unit circle ($|\lambda_i| < 1$), mathematically confirming that the state decay dynamics are contractive and prevent any explosive recurrent behavior.
...
Training throughput of HGDM at $2{,}048$ sequence length is marginally lower ($1.12$ steps/sec vs.\ $1.69$ steps/sec for the Transformer baseline) due to Triton GPU kernel launch overheads and intermediate state accumulation. However, as shown in Figure~\ref{fig:throughput}, this overhead is constant, and the parallel associative prefix scan's advantage manifests at longer context lengths ($8{,}192$ and above) where standard attention encounters severe overhead or Out-of-Memory (OOM) failures.
...
Neuroscience validation (encoding model analysis against ECoG data),
sparse-aware Triton kernel, scaling to 1B parameters, and multimodal
pretraining are planned.
...
experiments support reproducibility. Table~\ref{tab:mapping} provides a complete 
