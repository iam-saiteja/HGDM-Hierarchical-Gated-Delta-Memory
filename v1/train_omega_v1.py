...
def train_omega_v1():
    ...
    for step in range(start_step, args.steps):
        ...
        # Logging
        if (step <= 100 and step % 10 == 0) or (step > 100 and step % 500 == 0):
            dt = time.time() - t0
            t0 = time.time()
            ...
            print(f"Omega v1 Step {step:6d} | Loss: {true_ce_loss:.4f} | BPB: {bpb:.4f} | Tokens: {tokens_trained/1e6:.2f}M | VRAM: {gpu_mem}MB | Temp: {gpu_temp} | Time: {dt:.2f}s")
